"""Compact FM multitask model for `dataset_big10`, version 0_1.

Architecture:
- Raw waveform input
- Compact 1D CNN backbone over the raw waveform
- Shared dense trunk with categorical heads for `algorithm` and `ratio_carrier`
- A regression head over `log2(ratio_carrier)` and one over `log2(frequencia_base)` for smoother targets
...
OUTPUT_DIR = Path(os.getenv("OUTPUT_DIR", "model_training_big10_fmsynth3_0_1"))
MODEL_NAME = "model_training_big10_fmsynth3_0_1"
...
CNN_BATCH_DEFAULT = os.getenv("TRAIN_BATCH_SIZE", "24")
BATCH_SIZE = int(CNN_BATCH_DEFAULT)
PRED_BATCH_SIZE = int(os.getenv("PRED_BATCH_SIZE", "48"))
...
def build_model(input_len: int, n_algorithm_classes: int, n_ratio_classes: int) -> Model:
    ...
    return Model(
        ...
        name="compact_rawwave_multitask_big10_0_1",
    )
