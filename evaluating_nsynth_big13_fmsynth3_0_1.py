"""Evaluate NSynth resynthesis against stronger NSynth baselines for big13.
...
- Compares the model output against two baselines:
  ...
  - a pitch-matched harmonic waveform with several partials

Data flow:
- Input: `nsynth-test/audio`, `nsynth-test/examples.json`, and `nsynth-pred-big13/`
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evaluate NSynth resynthesis and stronger sine baselines.")
    parser.add_argument("--test-audio-dir", type=Path, default=Path("nsynth-test/audio"))
    parser.add_argument("--pred-audio-dir", type=Path, default=Path("nsynth-pred-big13"))
    parser.add_argument("--examples-json", type=Path, default=Path("nsynth-test/examples.json"))
    parser.add_argument("--output-dir", type=Path, default=Path("evaluation_nsynth_big13"))
    parser.add_argument(
...
def main() -> None:
    ...
    for file_name in test_files:
        ...
        harmonic_baseline = harmonic_wave(pitch_freq, len(reference), harmonics=5)
        ...
        harmonic_rows.append(
            {
                ...
                "pitch": pitch,
                "pitch_hz": pitch_freq,
                **compare_pair(reference, harmonic_baseline),
            }
        )
    ...
    payload = {
        ...
        "harmonic_baseline_vs_nsynth": harmonic_rows,
    }
    if args.include_440_baseline:
