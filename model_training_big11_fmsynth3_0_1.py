"""Algorithm-focused FM classifier for `dataset_big11`, version 0_1.
...
Data flow:
- Input: `dataset_big11/parameters.csv` plus `audio_big11_manifest.json` shards or `sample_*.wav`
...
BASE_PATH = Path(os.getenv("DATASET_PATH", "dataset_big11"))
OUTPUT_DIR = Path(os.getenv("OUTPUT_DIR", "model_training_big11_fmsynth3_0_1"))
MODEL_NAME = "model_training_big11_fmsynth3_0_1"
...
AUDIO_MANIFEST_PATH = BASE_PATH / "audio_big11_manifest.json"
AUDIO_LEGACY_CACHE_PATH = BASE_PATH / "audio_big11_int16.npy"
...
def build_model(
    ...
    return Model(
        ...
        name="compact_multirespectral_multitask_big11_0_1",
    )
...
def main() -> None:
    ...
    if not required_columns.issubset(target_raw.columns):
        raise ValueError("dataset_big11 must contain an `algorithm` column.")
    ...
    y_train_full.to_csv(OUTPUT_DIR / "y_train_big11.csv", index=False)
    y_test_full.to_csv(OUTPUT_DIR / "y_test_big11.csv", index=False)
