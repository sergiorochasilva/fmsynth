"""Balanced multi-head waveform regressor for `dataset_big7` (variant 0_1).

Architecture:
- Raw waveform input loaded from sharded `int16` audio or legacy caches
- Wider 1D residual CNN backbone with a multi-kernel stem, early downsampling, and multi-scale pooled features
- Shared dense trunk plus specialized heads for grouped numeric targets and categorical targets

Data flow:
- Input: `parameters.csv` plus `audio_big7_manifest.json` shards or `sample_*.wav`
- Output: trained model `.keras`, resumable checkpoints, target preprocessing bundle, predictions, plots, and `results.json`
"""
...
BASE_PATH = os.getenv("DATASET_PATH", "dataset_big7")
AUDIO_MANIFEST_PATH = os.path.join(BASE_PATH, "audio_big7_manifest.json")
AUDIO_LEGACY_CACHE_PATH = os.path.join(BASE_PATH, "audio_big7_int16.npy")
OUTPUT_DIR = "model_training_big7_fmsynth3_0_1"
MODEL_NAME = "model_training_big7_fmsynth3_0_1"
...
TOTAL_EPOCHS = int(os.getenv("EPOCHS", "220"))
RESUME_TRAINING = os.getenv("RESUME_TRAINING", "1") == "1"
CHECKPOINT_DIR = os.path.join(OUTPUT_DIR, "checkpoints")
CHECKPOINT_LATEST_WEIGHTS = os.path.join(CHECKPOINT_DIR, "latest.weights.h5")
CHECKPOINT_BEST_WEIGHTS = os.path.join(CHECKPOINT_DIR, "best.weights.h5")
TRAIN_STATE_PATH = os.path.join(CHECKPOINT_DIR, "training_state.json")

# Configuração de lote
# Batch intermediário para subir o uso de VRAM sem sair da faixa local.
...
LEARNING_RATE = float(os.getenv("LEARNING_RATE", "0.00012"))
...
def atomic_json_dump(path: str, payload: dict):
    tmp_path = f"{path}.tmp"
...
class ResumableCheckpointCallback(tf.keras.callbacks.Callback):
    def __init__(self, latest_weights_path: str, best_weights_path: str, state_path: str, initial_state: dict | None = None):
        super().__init__()
        ...
        if initial_state:
            best_val_loss = initial_state.get("best_val_loss")
            best_val_loss_epoch = initial_state.get("best_val_loss_epoch")
            if best_val_loss is not None:
                self.best_val_loss = float(best_val_loss)
            if best_val_loss_epoch is not None:
                self.best_val_loss_epoch = int(best_val_loss_epoch)
    ...
    def on_train_end(self, logs=None):
        # Garante que o último estado fique em disco mesmo quando o treino termina sem interrupção.
        self.model.save_weights(self.latest_weights_path)
...
# Split treino / validação com estratificação sobre algoritmo.
...
np.save(
    os.path.join(OUTPUT_DIR, "y_train_big7.npy"),
    y_train_full.to_numpy(dtype=np.float32),
)
np.save(
    os.path.join(OUTPUT_DIR, "y_test_big7.npy"),
...
def build_model(
    ...
    # Waveform branch with a multi-kernel stem for a bit more context.
    ...
    wave = Concatenate(name="wave_stem_concat")([wave_k7, wave_k3])
    wave = SpatialDropout1D(0.04, name="wave_stem_dropout")(wave)
    ...
    return Model(input_layer, outputs, name="complete_multihead_balanced_0_6")
...
resume_epoch = 0
if RESUME_TRAINING and resume_state and os.path.exists(CHECKPOINT_LATEST_WEIGHTS):
    try:
        model.load_weights(CHECKPOINT_LATEST_WEIGHTS)
        resume_epoch = int(resume_state.get("last_completed_epoch", -1)) + 1
        print(
            "Retomando treino a partir de "
            f"epoch={resume_epoch} usando {CHECKPOINT_LATEST_WEIGHTS}"
        )
    except Exception as exc:
        print(f"Aviso: checkpoint de retomada não pôde ser carregado: {exc}")
        resume_state = None
        resume_epoch = 0
elif RESUME_TRAINING:
    print("Nenhum checkpoint anterior encontrado; iniciando treino do zero.")
...
if resume_epoch >= TOTAL_EPOCHS and os.path.exists(hist_path):
    print(
        "Treino já concluído para o horizonte atual de epochs; "
        "reaproveitando histórico salvo."
    )
...
preprocess_bundle = {
    ...
    "audio_shard_dir": "audio_big7_shards",
...
results = {
    ...
    "runtime_config": {
        ...
        "latest_checkpoint": CHECKPOINT_LATEST_WEIGHTS,
        "best_checkpoint": CHECKPOINT_BEST_WEIGHTS,
