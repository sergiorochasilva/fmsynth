"""Hierarchical FM predictors for `dataset_big16`, version 0_2.
...
- Separate training stages with explicit hierarchy:
  ...
  - `exact`: family-conditioned classifier with dedicated `series` and `parallel` subheads
...
MODEL_PREFIX = "model_training_big16_fmsynth3_0_2"
...
GROUP_SPECS = {
    ...
    "exact": [
        {"head": "algorithm_family_aux_head", "column": "algorithm_family", "transform": "categorical", "loss_weight": 0.35, "group": "exact"},
        {"head": "series_exact_head", "column": "algorithm_series", "transform": "categorical", "loss_weight": 1.0, "group": "exact"},
        {"head": "parallel_exact_head", "column": "algorithm_parallel", "transform": "categorical", "loss_weight": 1.0, "group": "exact"},
    ],
}
...
def build_model(input_len: int, n_classes: int, target_group: str | None = None, target_specs: list[dict] | None = None) -> Model:
    ...
    if target_group == "family":
        ...
        family_condition_input = Input(shape=(n_classes,), name="family_condition_input")
        ...
        family_aux = Dense(max(DENSE_UNITS // 2, 96), activation="swish", name="exact_family_aux_dense")(exact_seq)
        family_aux = LayerNormalization(name="exact_family_aux_ln")(family_aux)
        family_aux = Dropout(EXACT_BRANCH_DROPOUT, name="exact_family_aux_drop")(family_aux)
        outputs.append(Dense(n_classes, activation="softmax", name="algorithm_family_aux_head")(family_aux))

        series_branch = Dense(max(EXACT_BRANCH_DENSE_UNITS // 2, 160), activation="swish", name="series_exact_dense_1")(exact_seq)
        ...
        series_branch = Dense(max(EXACT_BRANCH_DENSE_UNITS // 3, 96), activation="swish", name="series_exact_dense_2")(series_branch)
        ...
        outputs.append(Dense(2, activation="softmax", name="series_exact_head")(series_branch))

        parallel_branch = Dense(max(EXACT_BRANCH_DENSE_UNITS // 2, 160), activation="swish", name="parallel_exact_dense_1")(exact_seq)
        ...
        parallel_branch = Dense(max(EXACT_BRANCH_DENSE_UNITS // 3, 96), activation="swish", name="parallel_exact_dense_2")(parallel_branch)
        ...
        outputs.append(Dense(2, activation="softmax", name="parallel_exact_head")(parallel_branch))
    else:
...
def main() -> None:
    ...
    family_name_by_idx = {idx: name for name, idx in family_map.items()}
    series_family_name = next((name for name in family_classes if name == "series"), family_name_by_idx.get(0, family_classes[0]))
    parallel_family_name = next((name for name in family_classes if name == "parallel"), family_name_by_idx.get(1, family_classes[-1]))
    series_exact_classes = sorted(target_raw.loc[target_raw["algorithm_family"] == series_family_name, "algorithm"].unique().tolist())
    parallel_exact_classes = sorted(target_raw.loc[target_raw["algorithm_family"] == parallel_family_name, "algorithm"].unique().tolist())
    series_exact_map = {name: idx for idx, name in enumerate(series_exact_classes)}
    parallel_exact_map = {name: idx for idx, name in enumerate(parallel_exact_classes)}
    target_raw["algorithm_series_idx"] = target_raw["algorithm"].map(series_exact_map).fillna(0).astype(np.int32)
    target_raw["algorithm_parallel_idx"] = target_raw["algorithm"].map(parallel_exact_map).fillna(0).astype(np.int32)
    ...
    if group == "family":
        ...
        family_train = y_train_model["algorithm_family_idx"].to_numpy(dtype=np.int32)
        family_test = y_test_full["algorithm_family_idx"].to_numpy(dtype=np.int32)
        ...
        series_train = y_train_model["algorithm_series_idx"].to_numpy(dtype=np.int32)
        parallel_train = y_train_model["algorithm_parallel_idx"].to_numpy(dtype=np.int32)
        series_test = y_test_full["algorithm_series_idx"].to_numpy(dtype=np.int32)
        parallel_test = y_test_full["algorithm_parallel_idx"].to_numpy(dtype=np.int32)

        y_fit = {
            "algorithm_family_aux_head": family_train[fit_idx],
            "series_exact_head": series_train[fit_idx],
            "parallel_exact_head": parallel_train[fit_idx],
        }
        y_val = {
            "algorithm_family_aux_head": family_train[val_idx],
            "series_exact_head": series_train[val_idx],
            "parallel_exact_head": parallel_train[val_idx],
        }
        y_test = {
            "algorithm_family_aux_head": family_test,
            "series_exact_head": series_test,
            "parallel_exact_head": parallel_test,
        }
        sample_weight_fit = {
            "algorithm_family_aux_head": np.ones(len(fit_idx), dtype=np.float32),
            "series_exact_head": (family_train[fit_idx] == family_map[series_family_name]).astype(np.float32),
            "parallel_exact_head": (family_train[fit_idx] == family_map[parallel_family_name]).astype(np.float32),
        }
        sample_weight_val = {
            "algorithm_family_aux_head": np.ones(len(val_idx), dtype=np.float32),
            "series_exact_head": (family_train[val_idx] == family_map[series_family_name]).astype(np.float32),
            "parallel_exact_head": (family_train[val_idx] == family_map[parallel_family_name]).astype(np.float32),
        }
        sample_weight_test = {
            "algorithm_family_aux_head": np.ones(len(test_idx), dtype=np.float32),
            "series_exact_head": (family_test == family_map[series_family_name]).astype(np.float32),
            "parallel_exact_head": (family_test == family_map[parallel_family_name]).astype(np.float32),
        }
    ...
    class SplitSequence(tf.keras.utils.Sequence):
        ...
        def __getitem__(self, idx):
            ...
            if isinstance(self.y, dict):
                y_batch = tuple(values[batch_ids] for name, values in self.y.items())
            else:
                y_batch = self.y[batch_ids]
            weight_batch = None
            if self.sample_weights is not None:
                weight_batch = tuple(values[batch_ids] for name, values in self.sample_weights.items())
            if not self.conditions:
                if weight_batch is None:
                    return audio_batch, y_batch
                return audio_batch, y_batch, weight_batch
            x_batch = {"audio_input": audio_batch}
            for name, values in self.conditions.items():
                x_batch[name] = values[batch_ids]
            if weight_batch is None:
    ...
    train_seq = SplitSequence(audio_store, fit_audio_indices, y_fit, cond_fit, BATCH_SIZE, shuffle=True, sample_weights=sample_weight_fit)
    val_seq = SplitSequence(audio_store, val_audio_indices, y_val, cond_val, PRED_BATCH_SIZE, shuffle=False, sample_weights=sample_weight_val)
    test_seq = SplitSequence(audio_store, test_audio_indices, y_test, cond_test, PRED_BATCH_SIZE, shuffle=False, sample_weights=sample_weight_test)
    ...
    if group == "family":
        model.compile(
            ...
            loss=sparse_categorical_focal_loss(gamma=1.1),
        ...
        model.compile(
            optimizer=tf.keras.optimizers.Adam(learning_rate=LEARNING_RATE),
            loss={
                "algorithm_family_aux_head": tf.keras.losses.SparseCategoricalCrossentropy(),
                "series_exact_head": tf.keras.losses.SparseCategoricalCrossentropy(),
                "parallel_exact_head": tf.keras.losses.SparseCategoricalCrossentropy(),
            },
            loss_weights={
                "algorithm_family_aux_head": 0.35,
                "series_exact_head": 1.0,
                "parallel_exact_head": 1.0,
            },
            metrics={
                "algorithm_family_aux_head": "sparse_categorical_accuracy",
                "series_exact_head": "sparse_categorical_accuracy",
                "parallel_exact_head": "sparse_categorical_accuracy",
            },
        )
    ...
    class ResumableCheckpointCallback(tf.keras.callbacks.Callback):
        def __init__(self, latest_weights_path: Path, best_weights_path: Path, state_path: Path, initial_state: dict | None = None):
            ...
            best_val_loss = initial_state.get("best_val_loss") if initial_state else None
            best_val_epoch = initial_state.get("best_val_loss_epoch") if initial_state else None
            self.best_val_loss = None if best_val_loss in (None, "None") else float(best_val_loss)
            self.best_val_loss_epoch = None if best_val_epoch in (None, "None") else int(best_val_epoch)
    ...
    preds_oracle_raw = model.predict(test_seq, verbose=0)
    final_metrics = {}
    final_predictions = {"sample_id": test_audio_indices.astype(int)}

    if group == "family":
        preds_oracle = np.asarray(preds_oracle_raw, dtype=np.float32)
        ...
        family_aux_pred, series_pred, parallel_pred = [np.asarray(arr, dtype=np.float32) for arr in preds_oracle_raw]
        exact_true = np.asarray(y_test_full["algorithm_idx"], dtype=np.int32)
        family_true = np.asarray(y_test_full["algorithm_family_idx"], dtype=np.int32)
        series_true = np.asarray(y_test_full["algorithm_series_idx"], dtype=np.int32)
        parallel_true = np.asarray(y_test_full["algorithm_parallel_idx"], dtype=np.int32)

        series_global_indices = np.asarray([exact_map[name] for name in series_exact_classes], dtype=np.int32)
        parallel_global_indices = np.asarray([exact_map[name] for name in parallel_exact_classes], dtype=np.int32)

        family_pred_from_aux = np.argmax(family_aux_pred, axis=1)
        oracle_idx = np.where(
            family_true == family_map[series_family_name],
            series_global_indices[np.argmax(series_pred, axis=1)],
            parallel_global_indices[np.argmax(parallel_pred, axis=1)],
        ).astype(np.int32)

        final_predictions["algorithm_true"] = [exact_classes[i] for i in exact_true]
        final_predictions["algorithm_family_true"] = [family_classes[i] for i in family_true]
        final_predictions["algorithm_family_pred_aux"] = [family_classes[i] for i in family_pred_from_aux]
        final_predictions["algorithm_pred_oracle"] = [exact_classes[i] for i in oracle_idx]
        final_predictions["algorithm_prob_oracle"] = np.where(
            family_true == family_map[series_family_name],
            np.max(series_pred, axis=1),
            np.max(parallel_pred, axis=1),
        )
        final_metrics["family_accuracy_aux"] = float(np.mean(family_pred_from_aux == family_true))
        final_metrics["family_crossentropy_aux"] = float(tf.keras.losses.sparse_categorical_crossentropy(family_true, family_aux_pred).numpy().mean())
        series_mask = family_true == family_map[series_family_name]
        parallel_mask = family_true == family_map[parallel_family_name]
        final_metrics["series_accuracy_oracle"] = float(np.mean(np.argmax(series_pred[series_mask], axis=1) == series_true[series_mask])) if np.any(series_mask) else float("nan")
        final_metrics["parallel_accuracy_oracle"] = float(np.mean(np.argmax(parallel_pred[parallel_mask], axis=1) == parallel_true[parallel_mask])) if np.any(parallel_mask) else float("nan")
        final_metrics["algorithm_accuracy_oracle"] = float(np.mean(oracle_idx == exact_true))
        final_metrics["algorithm_crossentropy_oracle"] = float(
            np.mean(
                [
                    -np.log(np.clip(series_pred[i, series_true[i]], EPS, 1.0)) if family_true[i] == family_map[series_family_name] else -np.log(np.clip(parallel_pred[i, parallel_true[i]], EPS, 1.0))
                    for i in range(len(exact_true))
                ]
            )
        )
        ...
        if family_weights_path.exists():
            try:
                ...
                family_model.load_weights(str(family_weights_path), skip_mismatch=True)
                ...
                cascade_preds_raw = model.predict(cascade_seq, verbose=0)
                cascade_family_aux, cascade_series_pred, cascade_parallel_pred = [np.asarray(arr, dtype=np.float32) for arr in cascade_preds_raw]
                cascade_idx = np.where(
                    predicted_family_idx == family_map[series_family_name],
                    series_global_indices[np.argmax(cascade_series_pred, axis=1)],
                    parallel_global_indices[np.argmax(cascade_parallel_pred, axis=1)],
                ).astype(np.int32)
                final_predictions["family_pred_cascade"] = [family_classes[i] for i in predicted_family_idx]
                final_predictions["family_prob_cascade"] = np.max(family_preds, axis=1)
                final_predictions["algorithm_pred_cascade"] = [exact_classes[i] for i in cascade_idx]
                final_predictions["algorithm_family_pred_aux_cascade"] = [family_classes[i] for i in np.argmax(cascade_family_aux, axis=1)]
                final_predictions["algorithm_prob_cascade"] = np.where(
                    predicted_family_idx == family_map[series_family_name],
                    np.max(cascade_series_pred, axis=1),
                    np.max(cascade_parallel_pred, axis=1),
                )
                ...
                final_metrics["family_accuracy_cascade_aux"] = float(np.mean(np.argmax(cascade_family_aux, axis=1) == np.asarray(y_test_full["algorithm_family_idx"], dtype=np.int32)))
                final_metrics["algorithm_crossentropy_cascade"] = float(
                    np.mean(
                        [
                            -np.log(np.clip(cascade_series_pred[i, series_true[i]], EPS, 1.0)) if predicted_family_idx[i] == family_map[series_family_name] else -np.log(np.clip(cascade_parallel_pred[i, parallel_true[i]], EPS, 1.0))
                            for i in range(len(exact_true))
                        ]
                    )
                )
    ...
    results = {
        ...
        "series_exact_classes": series_exact_classes,
        "parallel_exact_classes": parallel_exact_classes,
    ...
    with open(output_dir / "algorithm_map.json", "w", encoding="utf-8") as f:
        json.dump(make_json_safe({
            ...
            "series_exact_classes": series_exact_classes,
            "series_exact_mapping": series_exact_map,
            "parallel_exact_classes": parallel_exact_classes,
            "parallel_exact_mapping": parallel_exact_map,
