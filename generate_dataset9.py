...
- Structural FM indices are kept non-zero so that the algorithm topologies remain acoustically distinct
...
- Output: `dataset_big9/parameters.csv`, `sample_*.wav`, `audio_big9_shards/shard_*.npy`, `audio_big9_manifest.json`, and `meta.json`
"""
...
OUTPUT_DIR = Path("dataset_big9")
SHARD_DIR = OUTPUT_DIR / "audio_big9_shards"
MANIFEST_PATH = OUTPUT_DIR / "audio_big9_manifest.json"
...
TARGET_INFO = {
    "dataset": "dataset_big9",
    "description": "compact benchmark with reintroduced structural FM differences and ratio target",
...
def write_audio(rows: list[dict], audio_sample_len: int) -> dict:
    ...
    for shard_idx in range(n_shards):
        ...
        shards_meta.append(
            {
                "index": shard_idx,
                "file": str(Path("audio_big9_shards") / shard_filename(shard_idx)),
