"""Generate a larger FM-synthesis benchmark corpus for robustness tests.
...
- Output: `dataset_big12/parameters.csv`, `sample_*.wav`, `audio_big12_shards/shard_*.npy`, `audio_big12_manifest.json`, and `meta.json`
"""
...
OUTPUT_DIR = Path("dataset_big12")
SHARD_DIR = OUTPUT_DIR / "audio_big12_shards"
MANIFEST_PATH = OUTPUT_DIR / "audio_big12_manifest.json"
...
TARGET_INFO = {
    "dataset": "dataset_big12",
    "description": "larger benchmark with balanced algorithm/ratio coverage, fixed nuisance controls, and 4-second clips",
...
def render_shard(task: tuple[int, list[dict], int]) -> dict:
    ...
    return {
        "index": shard_idx,
        "file": str(Path("audio_big12_shards") / shard_filename(shard_idx)),
