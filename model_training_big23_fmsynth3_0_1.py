...
- Extra temporal derivative (`delta2`) channel to emphasize algorithmic transients
...
MODEL_NAME = "model_training_big23_fmsynth3_0_1"
...
ALGO_BRANCH_FILTERS = int(os.getenv("ALGO_BRANCH_FILTERS", "192"))
ALGO_BRANCH_BLOCKS = int(os.getenv("ALGO_BRANCH_BLOCKS", "5"))
ALGO_BRANCH_DENSE_UNITS = int(os.getenv("ALGO_BRANCH_DENSE_UNITS", "512"))
...
def sparse_categorical_focal_loss(gamma: float = 1.2):
...
def build_model(input_len: int, n_algorithm_classes: int) -> Model:
    ...
    x = Lambda(logmel_frontend, output_shape=(None, N_MELS * 5), name="logmel_frontend")(audio_input)
    ...
    return Model(inputs=audio_input, outputs=outputs, name="algorithm_classifier_big23")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the big23 algorithm classifier.")
...
def main() -> None:
    ...
    model.compile(
        ...
        loss=sparse_categorical_focal_loss(gamma=1.2),
