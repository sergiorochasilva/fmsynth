...
- Output: `dataset_big11/parameters.csv`, `sample_*.wav`, `audio_big11_shards/shard_*.npy`, `audio_big11_manifest.json`, and `meta.json`
"""
...
OUTPUT_DIR = Path("dataset_big11")
SHARD_DIR = OUTPUT_DIR / "audio_big11_shards"
MANIFEST_PATH = OUTPUT_DIR / "audio_big11_manifest.json"
...
def write_audio(rows: list[dict], audio_sample_len: int) -> dict:
    ...
    for shard_idx in range(n_shards):
        ...
        shards_meta.append(
            {
                "index": shard_idx,
                "file": str(Path("audio_big11_shards") / shard_filename(shard_idx)),
