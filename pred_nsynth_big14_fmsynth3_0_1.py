"""Predict FM parameters for NSynth using the split `big14` models.
...
- Loads three separately trained models:
  ...
  - `carrier`
  - `modulators`
- Predicts each subspace independently and merges the outputs into a single parameter table

Data flow:
- Input: `nsynth-test/audio`, `nsynth-test/examples.json`, and the `big14` split model directories
...
from model_training_big14_fmsynth3_0_1 import (
    GROUP_SPECS,
    MODEL_PREFIX,
    build_model,
...
MODEL_NAME = "model_training_big14_fmsynth3_0_1"
...
DEFAULT_OUTPUT_DIR = Path("nsynth-pred-big14_0_1")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Predict NSynth parameters with the split big14 models.")
...
def main() -> None:
    ...
    for start in range(0, len(wav_files), args.batch_size):
        ...
        carrier_pred = carrier_model.predict(x_batch, batch_size=len(batch_files), verbose=0)
        mod_pred = mod_model.predict(x_batch, batch_size=len(batch_files), verbose=0)
