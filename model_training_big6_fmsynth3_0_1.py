"""Deep multi-head CNN regressor for `dataset_big6` (variant 0_1).

Architecture:
- Raw waveform input loaded from a memmap-friendly `int16` cache
- Deeper 1D CNN stack with smaller kernels, lower strides, and reduced pooling
...
OUTPUT_DIR = "model_training_big6_fmsynth3_0_1"
MODEL_NAME = "model_training_big6_fmsynth3_0_1"
...
train_audio_indices = train_idx.to_numpy(dtype=np.int32)
test_audio_indices = test_idx.to_numpy(dtype=np.int32)
...
def build_model(
    ...
    # Backbone mais profundo, com kernels menores e downsampling mais tardio.
    x_n = Conv1D(
        filters=16,
        kernel_size=7,
        strides=1,
