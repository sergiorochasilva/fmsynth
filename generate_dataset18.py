"""Generate an envelope-focused FM-synthesis benchmark corpus for hierarchical learning.
...
- Envelope parameters are sampled from structured archetypes so the ADSR stage is easier to learn
...
- Output: `dataset_big18/parameters.csv`, `sample_*.wav`, `audio_big18_shards/shard_*.npy`, `audio_big18_manifest.json`, and `meta.json`
"""
...
OUTPUT_DIR = Path("dataset_big18")
SHARD_DIR = OUTPUT_DIR / "audio_big18_shards"
MANIFEST_PATH = OUTPUT_DIR / "audio_big18_manifest.json"
...
TARGET_INFO = {
    "dataset": "dataset_big18",
    "description": "envelope-focused benchmark with balanced algorithm/ratio coverage, hierarchical algorithm labels, structured ADSR archetypes, and 4-second clips",
...
def sample_env_from_profile(profile: str, *, carrier: bool) -> Envelope:
    if profile == "percussive":
        attack = (0.0015, 0.008 if not carrier else 0.012)
        decay = (0.025, 0.10 if not carrier else 0.14)
        sustain = (0.04, 0.22 if not carrier else 0.34)
        release = (0.015, 0.10 if not carrier else 0.16)
    elif profile == "pluck":
        attack = (0.0015, 0.012 if not carrier else 0.015)
        decay = (0.04, 0.18 if not carrier else 0.24)
        sustain = (0.08, 0.35 if not carrier else 0.50)
        release = (0.02, 0.16 if not carrier else 0.24)
    elif profile == "held":
        attack = (0.004, 0.03 if not carrier else 0.04)
        decay = (0.08, 0.26 if not carrier else 0.34)
        sustain = (0.32, 0.78 if not carrier else 0.88)
        release = (0.04, 0.22 if not carrier else 0.35)
    elif profile == "swell":
        attack = (0.02, 0.12 if not carrier else 0.18)
        decay = (0.06, 0.22 if not carrier else 0.30)
        sustain = (0.45, 0.90 if not carrier else 0.96)
        release = (0.08, 0.34 if not carrier else 0.55)
    else:
...
def sample_env_mod() -> Envelope:
    return sample_env_from_profile(choose_profile(ENV_MOD_PROFILE_WEIGHTS), carrier=False)


def sample_env_car() -> Envelope:
    return sample_env_from_profile(choose_profile(ENV_CAR_PROFILE_WEIGHTS), carrier=True)
...
def render_shard(task: tuple[int, list[dict], int]) -> dict:
    ...
    return {
        "index": shard_idx,
        "file": str(Path("audio_big18_shards") / shard_filename(shard_idx)),
