"""Multi-resolution spectral FM multitask model for `dataset_big10`, version 0_5.
...
- Shared dense trunk with a stronger categorical branch for `algorithm`
- Regression heads over `log2(ratio_carrier)` and `log2(frequencia_base)` for smoother targets
...
OUTPUT_DIR = Path(os.getenv("OUTPUT_DIR", "model_training_big10_fmsynth3_0_5"))
MODEL_NAME = "model_training_big10_fmsynth3_0_5"
...
CNN_BLOCKS = int(os.getenv("CNN_BLOCKS", "5"))
...
def logmel_frontend(x):
    ...
    delta_large = log_mel_large[:, 1:, :] - log_mel_large[:, :-1, :]
    delta_large = tf.pad(delta_large, paddings=[[0, 0], [1, 0], [0, 0]])
    delta_large = delta_large - tf.reduce_mean(delta_large, axis=[1, 2], keepdims=True)
    delta_large = delta_large / (tf.math.reduce_std(delta_large, axis=[1, 2], keepdims=True) + EPS)
    return tf.concat([log_mel_small, log_mel_large, delta_large], axis=-1)


def build_model(input_len: int, n_algorithm_classes: int) -> Model:
    inputs = Input(shape=(input_len, 1), name="audio_input")
    x = Lambda(logmel_frontend, output_shape=(None, N_MELS * 3), name="logmel_frontend")(inputs)
    ...
    for block_idx in range(CNN_BLOCKS):
        ...
        dilation = 1 if block_idx < 2 else 2 if block_idx < 4 else 4
        x = Conv1D(
            filters,
    ...
    algo_branch = Dense(max(DENSE_UNITS // 2, 128), activation="swish", name="algo_branch_dense")(x)
    ...
    ratio_log2_head = Dense(1, activation=None, dtype="float32", name="ratio_log2_head")(numeric_branch)
    ...
    return Model(
        ...
        name="compact_multirespectral_multitask_big10_0_5",
    )
...
def main() -> None:
    ...
    ratio_pred_idx = np.abs(ratio_classes[None, :] - ratio_pred_value[:, None]).argmin(axis=1)
