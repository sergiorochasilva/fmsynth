"""Generate a larger FM-synthesis benchmark corpus with richer controllable parameters.
...
- The main temporal controls vary across the corpus: envelope shape, feedback, LFO, key scaling, and carrier detune
...
- Output: `dataset_big13/parameters.csv`, `sample_*.wav`, `audio_big13_shards/shard_*.npy`, `audio_big13_manifest.json`, and `meta.json`
"""
...
OUTPUT_DIR = Path("dataset_big13")
SHARD_DIR = OUTPUT_DIR / "audio_big13_shards"
MANIFEST_PATH = OUTPUT_DIR / "audio_big13_manifest.json"
...
TARGET_INFO = {
    "dataset": "dataset_big13",
    "description": "larger benchmark with balanced algorithm/ratio coverage, variable envelopes and controls, and 4-second clips",
...
def sample_env_mod() -> Envelope:
    return Envelope(
        r(uniform_log(0.002, 0.08)),
        r(uniform_log(0.03, 0.55)),
        r(random.uniform(0.18, 0.88)),
        r(uniform_log(0.02, 0.65)),
        FIXED_CURVE,
        FIXED_CURVE,
        FIXED_CURVE,
    )
...
def sample_env_car() -> Envelope:
    return Envelope(
        r(uniform_log(0.001, 0.12)),
        r(uniform_log(0.04, 0.65)),
        r(random.uniform(0.12, 0.95)),
        r(uniform_log(0.03, 0.80)),
        FIXED_CURVE,
        FIXED_CURVE,
        FIXED_CURVE,
    )
...
def build_row(sample_id: int, algorithm: str, ratio_carrier: float) -> dict:
    ...
    while True:
        ...
        return {
            ...
            "detune_carrier": r(random.uniform(-12.0, 12.0)),
            "feedback": r(random.uniform(0.0, 0.45)),
            "lfo_rate": r(random.uniform(0.0, 9.0)),
            "lfo_depth_cents": r(random.uniform(0.0, 22.0)),
            "key_scaling": r(random.uniform(0.0, 0.80)),
            "env_mod_attack": 0.0,
            "env_mod_decay": 0.0,
            "env_mod_sustain": 0.0,
            "env_mod_release": 0.0,
...
def render_shard(task: tuple[int, list[dict], int]) -> dict:
    ...
    return {
        "index": shard_idx,
        "file": str(Path("audio_big13_shards") / shard_filename(shard_idx)),
