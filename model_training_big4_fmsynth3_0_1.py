"""CNN regressor for `dataset_big4` synthesis parameters.
...
OUTPUT_DIR = "model_training_big4_fmsynth3_0_1"
MODEL_NAME = "model_training_big4_fmsynth3_0_1"
...
np.save(os.path.join(OUTPUT_DIR, "x_train_big4.npy"), x)
np.save(os.path.join(OUTPUT_DIR, "y_train_big4.npy"), y_train_norm)
...
# Ajustando dimensão de X de teste
x_t = x_t.reshape((x_t.shape[0], x_t.shape[1], 1))

y_t = test.drop(columns=[0])

np.save(os.path.join(OUTPUT_DIR, "x_test_big4.npy"), x_t)
np.save(os.path.join(OUTPUT_DIR, "y_test_big4.npy"), y_t)
