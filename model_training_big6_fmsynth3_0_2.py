"""Deep multi-head CNN regressor for `dataset_big6` (variant 0_2).

Architecture:
- Raw waveform input loaded from sharded `int16` audio or legacy caches
- Deeper 1D CNN backbone with residual blocks, multiscale pooled features, and later downsampling
...
OUTPUT_DIR = "model_training_big6_fmsynth3_0_2"
MODEL_NAME = "model_training_big6_fmsynth3_0_2"
...
def build_model(
    ...
    # Backbone residual multi-escala, com kernels menores e downsampling mais tardio.
    x_n = Conv1D(
        filters=24,
        kernel_size=7,
        strides=1,
    ...
    x_n = SpatialDropout1D(0.04, name="stem_spatial_dropout")(x_n)
    ...
    x_n = MaxPooling1D(pool_size=2, name="pool_3")(block_2)

    block_3 = residual_conv_block(
        x_n,
        filters=72,
        kernel_size=3,
        activation=activation,
    ...
    x_n = MaxPooling1D(pool_size=2, name="pool_4")(block_4)

    block_5 = residual_conv_block(
        x_n,
        filters=128,
        kernel_size=3,
        activation=activation,
    ...
    def pooled_features(tensor, prefix):
    ...
    features = Concatenate(name="multi_scale_features")(
        [
            pooled_features(block_2, "scale1"),
            pooled_features(block_4, "scale2"),
            pooled_features(block_5, "scale3"),
        ]
    )
    ...
    shared_head = Dropout(max(dropout_head * 0.5, 0.08), name="shared_head_dropout_2")(shared_head)
    shared_head = Dense(
        128,
        activation=activation,
        use_bias=bias_head,
    ...
    for head_name, output_dim in numeric_output_dims.items():
        ...
        reg_hidden = Dropout(max(dropout_head * 0.5, 0.08), name=f"{head_name}_dropout")(reg_hidden)
        reg_hidden = Dense(
            96,
            activation=activation,
            use_bias=bias_head,
    ...
    if has_freq_head:
        ...
        freq_hidden = Dropout(max(dropout_head * 0.4, 0.05), name="freq_head_dropout")(freq_hidden)
    ...
    for col, n_classes in categorical_output_dims.items():
        ...
        cat_hidden = Dropout(max(dropout_head * 0.35, 0.05), name=f"{head_name}_dropout")(cat_hidden)
    ...
    return Model(input_layer, outputs, name="complete_multihead_cnn_0_7")
