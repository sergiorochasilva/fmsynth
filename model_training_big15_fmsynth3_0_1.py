...
- Separate training modes with explicit conditioning and teacher forcing:
  - `algorithm`: categorical classifier
  - `carrier`: carrier ratio / base frequency / detune regressors conditioned on true `algorithm`
  - `modulators`: FM indices, feedback, LFO, key scaling, and ADSR regressors conditioned on true `algorithm` and true `carrier`
...
MODEL_PREFIX = "model_training_big15_fmsynth3_0_1"
...
GROUP_SPECS = {
    ...
    "carrier": [
        {"head": "ratio_log2_head", "column": "ratio_carrier", "transform": "log2", "loss_weight": 0.85, "group": "carrier"},
        {"head": "freq_log2_head", "column": "frequencia_base", "transform": "log2", "loss_weight": 0.85, "group": "carrier"},
        {"head": "detune_carrier_head", "column": "detune_carrier", "transform": "identity", "loss_weight": 0.55, "group": "carrier"},
    ],
    "modulators": [
        {"head": "index_12_head", "column": "index_12", "transform": "identity", "loss_weight": 0.65, "group": "mod"},
        {"head": "index_23_head", "column": "index_23", "transform": "identity", "loss_weight": 0.65, "group": "mod"},
        {"head": "index_3c_head", "column": "index_3c", "transform": "identity", "loss_weight": 0.65, "group": "mod"},
        {"head": "index_4c_head", "column": "index_4c", "transform": "identity", "loss_weight": 0.65, "group": "mod"},
        {"head": "index_5c_head", "column": "index_5c", "transform": "identity", "loss_weight": 0.65, "group": "mod"},
        {"head": "feedback_head", "column": "feedback", "transform": "identity", "loss_weight": 0.50, "group": "mod"},
        {"head": "lfo_rate_head", "column": "lfo_rate", "transform": "identity", "loss_weight": 0.50, "group": "mod"},
        {"head": "lfo_depth_cents_head", "column": "lfo_depth_cents", "transform": "identity", "loss_weight": 0.50, "group": "mod"},
        {"head": "key_scaling_head", "column": "key_scaling", "transform": "identity", "loss_weight": 0.50, "group": "mod"},
        {"head": "env_mod_attack_head", "column": "env_mod_attack", "transform": "log2", "loss_weight": 0.60, "group": "env"},
        {"head": "env_mod_decay_head", "column": "env_mod_decay", "transform": "log2", "loss_weight": 0.60, "group": "env"},
        {"head": "env_mod_sustain_head", "column": "env_mod_sustain", "transform": "identity", "loss_weight": 0.50, "group": "env"},
        {"head": "env_mod_release_head", "column": "env_mod_release", "transform": "log2", "loss_weight": 0.60, "group": "env"},
        {"head": "env_car_attack_head", "column": "env_car_attack", "transform": "log2", "loss_weight": 0.60, "group": "env"},
        {"head": "env_car_decay_head", "column": "env_car_decay", "transform": "log2", "loss_weight": 0.60, "group": "env"},
        {"head": "env_car_sustain_head", "column": "env_car_sustain", "transform": "identity", "loss_weight": 0.50, "group": "env"},
        {"head": "env_car_release_head", "column": "env_car_release", "transform": "log2", "loss_weight": 0.60, "group": "env"},
    ],
}
...
CONDITION_HEADS = {
    "carrier": ["algorithm_onehot"],
    "modulators": ["algorithm_onehot", "carrier_condition"],
}
...
def load_audio_store_wavs(base_path: Path, sample_ids: list[int]) -> np.ndarray:
    samples = []
...
def build_model(input_len: int, n_algorithm_classes: int, target_group: str | None = None, target_specs: list[dict] | None = None) -> Model:
    ...
    if target_group == "algorithm":
        algo_joint = Dense(max(DENSE_UNITS, 192), activation="swish", name="algo_joint_dense")(x)
        ...
        carrier_branch = Concatenate(name="carrier_condition_concat")([x, algorithm_condition])
        carrier_branch = Dense(max(DENSE_UNITS // 2, 96), activation="swish", name="carrier_branch_dense")(carrier_branch)
        carrier_branch = LayerNormalization(name="carrier_branch_ln")(carrier_branch)
        carrier_branch = Dropout(DROPOUT, name="carrier_branch_drop")(carrier_branch)
        outputs.append(scalar_head(carrier_branch, "ratio_log2_head"))
        outputs.append(scalar_head(carrier_branch, "freq_log2_head"))
        outputs.append(scalar_head(carrier_branch, "detune_carrier_head"))
    else:
        algorithm_condition_input = Input(shape=(n_algorithm_classes,), name="algorithm_condition_input")
        carrier_condition_input = Input(shape=(3,), name="carrier_condition_input")
        input_tensors.extend([algorithm_condition_input, carrier_condition_input])
        algorithm_condition = Dense(max(DENSE_UNITS // 4, 64), activation="swish", name="algorithm_condition_dense")(algorithm_condition_input)
        algorithm_condition = LayerNormalization(name="algorithm_condition_ln")(algorithm_condition)
        carrier_condition = Dense(max(DENSE_UNITS // 4, 64), activation="swish", name="carrier_condition_dense")(carrier_condition_input)
        carrier_condition = LayerNormalization(name="carrier_condition_ln")(carrier_condition)
        mod_branch = Concatenate(name="mod_condition_concat")([x, algorithm_condition, carrier_condition])
        mod_branch = Dense(max(DENSE_UNITS // 2, 96), activation="swish", name="mod_branch_dense")(mod_branch)
        mod_branch = LayerNormalization(name="mod_branch_ln")(mod_branch)
        mod_branch = Dropout(DROPOUT, name="mod_branch_drop")(mod_branch)
        for spec in target_specs:
            outputs.append(scalar_head(mod_branch, spec["head"]))

    return Model(inputs=input_tensors, outputs=outputs, name=f"conditioned_split_fm_big15_{target_group}")
...
def group_stratify_key(frame: pd.DataFrame, target_group: str | None = None) -> pd.Series:
    ...
    if target_group == "carrier":
        ...
        return base + "__" + ratio_bucket.astype(str) + "__" + freq_bucket.astype(str)
    ratio_bucket = pd.qcut(frame["ratio_carrier"].rank(method="first"), q=4, labels=False, duplicates="drop")
    return base + "__" + ratio_bucket.astype(str)
...
def main() -> None:
    ...
    if group in {"carrier", "modulators"}:
    ...
    if group == "modulators":
        carrier_cond_specs = GROUP_SPECS["carrier"]
        carrier_cond_scalers: dict[str, StandardScaler] = {}
        carrier_fit_parts: list[np.ndarray] = []
        carrier_val_parts: list[np.ndarray] = []
        carrier_test_parts: list[np.ndarray] = []
        for spec in carrier_cond_specs:
            scaler = StandardScaler()
            carrier_cond_scalers[spec["head"]] = scaler
            train_values = y_train_model[spec["head"]].to_numpy(dtype=np.float32).reshape(-1, 1)
            fit_values = scaler.fit_transform(train_values).astype(np.float32)
            val_values = scaler.transform(y_train_model[spec["head"]].to_numpy(dtype=np.float32)[val_idx].reshape(-1, 1)).astype(np.float32)
            test_values = scaler.transform(y_test_full[spec["head"]].to_numpy(dtype=np.float32).reshape(-1, 1)).astype(np.float32)
            carrier_fit_parts.append(fit_values[fit_idx])
            carrier_val_parts.append(val_values)
            carrier_test_parts.append(test_values)
        cond_fit["carrier_condition_input"] = np.concatenate(carrier_fit_parts, axis=1)
        cond_val["carrier_condition_input"] = np.concatenate(carrier_val_parts, axis=1)
        cond_test["carrier_condition_input"] = np.concatenate(carrier_test_parts, axis=1)
        for spec in carrier_cond_specs:
            joblib.dump(carrier_cond_scalers[spec["head"]], output_dir / f"{spec['head']}_condition_scaler.joblib")
    ...
    class SplitSequence(tf.keras.utils.Sequence):
        def __init__(self, audio: np.ndarray, sample_indices: np.ndarray, y: dict[str, np.ndarray] | np.ndarray, conditions: dict[str, np.ndarray], batch_size: int, shuffle: bool = True):
            self.audio = np.asarray(audio, dtype=np.int16)
