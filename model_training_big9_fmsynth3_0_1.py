"""Compact FM multitask model for `dataset_big9`, version 0_1.

Architecture:
- Raw waveform input
- Compact 1D CNN backbone over the raw waveform
...
OUTPUT_DIR = Path(os.getenv("OUTPUT_DIR", "model_training_big9_fmsynth3_0_1"))
MODEL_NAME = "model_training_big9_fmsynth3_0_1"
...
def build_model(input_len: int, n_algorithm_classes: int, n_ratio_classes: int) -> Model:
    ...
    for block_idx in range(CNN_BLOCKS):
        x = Conv1D(filters, kernel_size=7, padding="same", activation="swish", name=f"conv_{block_idx + 1}_a")(x)
        x = BatchNormalization(name=f"bn_{block_idx + 1}_a")(x)
        x = Conv1D(filters, kernel_size=5, padding="same", activation="swish", name=f"conv_{block_idx + 1}_b")(x)
    ...
    ratio_head = Dense(n_ratio_classes, activation="softmax", name="ratio_head")(x)
    freq_log2_head = Dense(1, activation=None, dtype="float32", name="freq_log2_head")(x)

    return Model(
        inputs=inputs,
        outputs=[algorithm_head, ratio_head, freq_log2_head],
        name="compact_logmel_multitask_big9_0_1",
    )
...
def main() -> None:
    ...
    y_fit = {
        ...
        "ratio_head": y_train_model["ratio_idx"].to_numpy(dtype=np.int32)[fit_idx],
        "freq_log2_head": y_freq_fit,
    }
    y_val = {
        "algorithm_head": y_train_model["algorithm_idx"].to_numpy(dtype=np.int32)[val_idx],
        "ratio_head": y_train_model["ratio_idx"].to_numpy(dtype=np.int32)[val_idx],
    ...
    test_seq = MultiTaskSequence(
        ...
        test_audio_indices,
        {
            ...
            "ratio_head": y_test_full["ratio_idx"].to_numpy(dtype=np.int32),
    ...
    model.compile(
        ...
        loss={
            ...
            "ratio_head": tf.keras.losses.SparseCategoricalCrossentropy(),
            "freq_log2_head": tf.keras.losses.Huber(delta=1.0),
        },
        metrics={
            "algorithm_head": ["sparse_categorical_accuracy"],
            "ratio_head": ["sparse_categorical_accuracy"],
    ...
    print(
        ...
        f"base_filters={BASE_FILTERS}, blocks={CNN_BLOCKS}, ratio_class_count={len(ratio_classes)}, "
    ...
    ratio_acc = float(np.mean(ratio_pred_idx == ratio_true_idx))
    algo_ce = float(tf.keras.losses.sparse_categorical_crossentropy(algo_true, algo_pred).numpy().mean())
    ratio_ce = float(tf.keras.losses.sparse_categorical_crossentropy(ratio_true_idx, ratio_pred).numpy().mean())
    ...
    predictions = pd.DataFrame(
        {
            ...
            "ratio_carrier_prob": np.max(ratio_pred, axis=1),
    ...
    plot_metric(history_df, "ratio_head_sparse_categorical_accuracy", "val_ratio_head_sparse_categorical_accuracy", "Ratio Accuracy", "train_ratio_accuracy.png")
