"""Predict FM parameters for NSynth using the conditioned split `big15` models.

Architecture:
- Loads three separately trained models:
  - `algorithm`
  - `carrier` conditioned on the predicted `algorithm`
  - `modulators` conditioned on the predicted `algorithm` and predicted `carrier`
- Predicts the subspaces in cascade and merges the outputs into a single parameter table

Data flow:
- Input: `nsynth-test/audio`, `nsynth-test/examples.json`, and the `big15` split model directories
...
from model_training_big15_fmsynth3_0_1 import (
    GROUP_SPECS,
    MODEL_PREFIX,
    build_model,
...
MODEL_NAME = "model_training_big15_fmsynth3_0_1"
...
DEFAULT_OUTPUT_DIR = Path("nsynth-pred-big15_0_1")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Predict NSynth parameters with the conditioned split big15 models.")
...
def main() -> None:
    ...
    carrier_model, carrier_dir = load_model_for_group("carrier", audio_len, len(algorithm_classes))
    mod_model, mod_dir = load_model_for_group("modulators", audio_len, len(algorithm_classes))
    ...
    mod_scalers = {}
    for spec in GROUP_SPECS["modulators"]:
    ...
    for start in range(0, len(wav_files), args.batch_size):
        ...
        carrier_pred = carrier_model.predict({"audio_input": x_batch, "algorithm_condition_input": algo_onehot}, batch_size=len(batch_files), verbose=0)
        ...
        for idx, wav_path in enumerate(batch_files):
            ...
            for spec_idx, spec in enumerate(GROUP_SPECS["carrier"]):
            ...
            for spec_idx, spec in enumerate(GROUP_SPECS["modulators"]):
