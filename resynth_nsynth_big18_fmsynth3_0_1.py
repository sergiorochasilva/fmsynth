"""Resynthesize NSynth audio from the split `big18` `0_1` predictions.

Architecture:
- Loads the merged predictions from the `0_1` cascade
- Reconstructs the waveform with `FMSynth3`
- Uses dataset priors only for missing controls

Data flow:
- Input: predicted parameter JSON and NSynth metadata
- Output: resynthesized `.wav` files in `nsynth-pred-big18_0_1/`
"""
...
MODEL_TAG = "big18_0_1"
DEFAULT_PRED_JSON = Path("nsynth-pred-big18_0_1") / f"params_pred_nsynth_{MODEL_TAG}.json"
...
DEFAULT_DATASET_PARAMS = Path("dataset_big18/parameters.csv")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Resynthesize NSynth using split big18 predictions.")
