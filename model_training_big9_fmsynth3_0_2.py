"""Compact FM multitask model for `dataset_big9`, version 0_2.

Architecture:
- Raw waveform input
- Compact 1D CNN backbone over the raw waveform
- Shared dense trunk with a categorical head for `algorithm`
...
OUTPUT_DIR = Path(os.getenv("OUTPUT_DIR", "model_training_big9_fmsynth3_0_2"))
MODEL_NAME = "model_training_big9_fmsynth3_0_2"
...
def build_model(input_len: int, n_algorithm_classes: int) -> Model:
    ...
    for block_idx in range(CNN_BLOCKS):
        kernel_a = 9 if block_idx < 2 else 5 if block_idx < 4 else 3
        kernel_b = max(3, kernel_a - 2)
        x = Conv1D(filters, kernel_size=kernel_a, padding="same", activation="swish", name=f"conv_{block_idx + 1}_a")(x)
        x = BatchNormalization(name=f"bn_{block_idx + 1}_a")(x)
        x = Conv1D(filters, kernel_size=kernel_b, padding="same", activation="swish", name=f"conv_{block_idx + 1}_b")(x)
    ...
    return Model(
        ...
        name="compact_rawwave_multitask_big9_0_2",
    )
