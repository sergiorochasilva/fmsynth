"""Algorithm-focused FM classifier for `dataset_big24`, version 0_1.
...
Data flow:
- Input: `dataset_big24/parameters.csv` plus `audio_big24_manifest.json` shards or `sample_*.wav`
...
BASE_PATH = Path(os.getenv("DATASET_PATH", "dataset_big24"))
OUTPUT_DIR = Path(os.getenv("OUTPUT_DIR", "model_training_big24_fmsynth3_0_1"))
MODEL_NAME = "model_training_big24_fmsynth3_0_1"
...
AUDIO_MANIFEST_PATH = BASE_PATH / "audio_big24_manifest.json"
AUDIO_LEGACY_CACHE_PATH = BASE_PATH / "audio_big24_int16.npy"
...
def build_model(
    ...
    return Model(
        ...
        name="compact_multirespectral_multitask_big24_0_1",
    )
...
def main() -> None:
    ...
    if not required_columns.issubset(target_raw.columns):
        raise ValueError("dataset_big24 must contain an `algorithm` column.")
    ...
    y_train_full.to_csv(OUTPUT_DIR / "y_train_big24.csv", index=False)
    y_test_full.to_csv(OUTPUT_DIR / "y_test_big24.csv", index=False)
