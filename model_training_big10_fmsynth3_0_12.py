"""Algorithm-focused multi-task FM classifier for `dataset_big10`, version 0_12.
...
- Auxiliary classification heads for `algorithm` prefix and suffix structure
...
OUTPUT_DIR = Path(os.getenv("OUTPUT_DIR", "model_training_big10_fmsynth3_0_12"))
MODEL_NAME = "model_training_big10_fmsynth3_0_12"
...
def build_model(
    ...
    algo_joint = Concatenate(name="algo_joint_concat")([algo_branch, algo_prefix_branch, algo_suffix_branch, ratio_branch, freq_branch, x])
    ...
    return Model(
        inputs=inputs,
        outputs=[algorithm_head, algorithm_prefix_head, algorithm_suffix_head, ratio_log2_head, freq_log2_head],
        name="compact_multirespectral_multitask_big10_0_12",
    )
...
def main() -> None:
    ...
    y_fit = {
        "algorithm_head": tf.keras.utils.to_categorical(y_train_model["algorithm_idx"].to_numpy(dtype=np.int32)[fit_idx], num_classes=len(algorithm_classes)).astype(np.float32),
        "algorithm_prefix_head": tf.keras.utils.to_categorical(y_train_model["algorithm_prefix_idx"].to_numpy(dtype=np.int32)[fit_idx], num_classes=len(algorithm_prefix_classes)).astype(np.float32),
        "algorithm_suffix_head": tf.keras.utils.to_categorical(y_train_model["algorithm_suffix_idx"].to_numpy(dtype=np.int32)[fit_idx], num_classes=len(algorithm_suffix_classes)).astype(np.float32),
        "ratio_log2_head": y_ratio_train[fit_idx].astype(np.float32),
        "freq_log2_head": y_freq_train[fit_idx].astype(np.float32),
    }
    y_val = {
        "algorithm_head": tf.keras.utils.to_categorical(y_train_model["algorithm_idx"].to_numpy(dtype=np.int32)[val_idx], num_classes=len(algorithm_classes)).astype(np.float32),
        "algorithm_prefix_head": tf.keras.utils.to_categorical(y_train_model["algorithm_prefix_idx"].to_numpy(dtype=np.int32)[val_idx], num_classes=len(algorithm_prefix_classes)).astype(np.float32),
        "algorithm_suffix_head": tf.keras.utils.to_categorical(y_train_model["algorithm_suffix_idx"].to_numpy(dtype=np.int32)[val_idx], num_classes=len(algorithm_suffix_classes)).astype(np.float32),
    ...
    test_seq = MultiTaskSequence(
        ...
        test_audio_indices,
        {
            "algorithm_head": tf.keras.utils.to_categorical(y_test_full["algorithm_idx"].to_numpy(dtype=np.int32), num_classes=len(algorithm_classes)).astype(np.float32),
            "algorithm_prefix_head": tf.keras.utils.to_categorical(y_test_full["algorithm_prefix_idx"].to_numpy(dtype=np.int32), num_classes=len(algorithm_prefix_classes)).astype(np.float32),
            "algorithm_suffix_head": tf.keras.utils.to_categorical(y_test_full["algorithm_suffix_idx"].to_numpy(dtype=np.int32), num_classes=len(algorithm_suffix_classes)).astype(np.float32),
    ...
    model.compile(
        ...
        loss={
            "algorithm_head": tf.keras.losses.CategoricalCrossentropy(label_smoothing=LABEL_SMOOTHING),
            "algorithm_prefix_head": tf.keras.losses.CategoricalCrossentropy(label_smoothing=PREFIX_SUFFIX_LABEL_SMOOTHING),
            "algorithm_suffix_head": tf.keras.losses.CategoricalCrossentropy(label_smoothing=PREFIX_SUFFIX_LABEL_SMOOTHING),
            "ratio_log2_head": tf.keras.losses.Huber(delta=1.0),
            "freq_log2_head": tf.keras.losses.Huber(delta=1.0),
        },
        metrics={
            "algorithm_head": ["categorical_accuracy"],
            "algorithm_prefix_head": ["categorical_accuracy"],
            "algorithm_suffix_head": ["categorical_accuracy"],
            "ratio_log2_head": ["mae", "mse"],
            "freq_log2_head": ["mae", "mse"],
        },
        loss_weights={
            "algorithm_head": 2.4,
            "algorithm_prefix_head": 0.8,
            "algorithm_suffix_head": 0.8,
    ...
    algo_ce = float(
        tf.keras.losses.categorical_crossentropy(
            tf.keras.utils.to_categorical(algo_true, num_classes=len(algorithm_classes)),
            algo_pred,
        ).numpy().mean()
    )
    ...
    plot_metric(history_df, "algorithm_head_categorical_accuracy", "val_algorithm_head_categorical_accuracy", "Algorithm Accuracy", "train_algorithm_accuracy.png")
    ...
    results = {
        ...
        "runtime_config": {
            ...
            "prefix_suffix_label_smoothing": PREFIX_SUFFIX_LABEL_SMOOTHING,
        },
