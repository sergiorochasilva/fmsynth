"""Resynthesize NSynth audio from `big12` predictions.

Architecture:
- Loads predicted algorithm / ratio / carrier frequency from the `big12` model
- Fills the remaining FM structural indices using corpus priors conditioned on algorithm and ratio
- Renders waveforms with `FMSynth3`

Data flow:
- Input: predicted parameter JSON and NSynth metadata
- Output: resynthesized `.wav` files in `nsynth-pred/`
"""
...
DEFAULT_PRED_JSON = MODEL_DIR / "nsynth-pred" / f"params_pred_nsynth_{MODEL_NAME}.json"
DEFAULT_NSYNTH_AUDIO_DIR = Path("nsynth-test/audio")
DEFAULT_EXAMPLES_JSON = Path("nsynth-test/examples.json")
DEFAULT_OUTPUT_DIR = Path("nsynth-pred")
DEFAULT_DATASET_PARAMS = Path("dataset_big12/parameters.csv")
...
FIXED_RATIO_4 = 3.0
FIXED_RATIO_5 = 4.0
FIXED_ENV_MOD = Envelope(0.01, 0.14, 0.68, 0.18, "exp", "exp", "exp")
FIXED_ENV_CAR = Envelope(0.01, 0.16, 0.75, 0.20, "exp", "exp", "exp")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Resynthesize NSynth using big12 predictions.")
...
def build_prior_table(dataset_params: Path) -> dict[str, dict[str, dict[str, float]]]:
    df = pd.read_csv(dataset_params)
    priors: dict[str, dict[str, dict[str, float]]] = {"_global": {}}
    ratio_labels = sorted({round(float(value), 4) for value in df["ratio_carrier"].astype(float).unique().tolist()})

    for ratio_value in ratio_labels:
        key = f"{ratio_value:.4f}"
        subset = df[np.isclose(df["ratio_carrier"].astype(float), ratio_value, atol=1e-4)]
        if subset.empty:
            continue
        priors["_global"][key] = {
            "index_12": float(subset["index_12"].median()),
            "index_23": float(subset["index_23"].median()),
            "index_3c": float(subset["index_3c"].median()),
            "index_4c": float(subset["index_4c"].median()),
            "index_5c": float(subset["index_5c"].median()),
        }

    for algorithm, group in df.groupby("algorithm"):
        algo_map: dict[str, dict[str, float]] = {}
        algo_map["_global"] = {
            "index_12": float(group["index_12"].median()),
            "index_23": float(group["index_23"].median()),
            "index_3c": float(group["index_3c"].median()),
            "index_4c": float(group["index_4c"].median()),
            "index_5c": float(group["index_5c"].median()),
        }
        for ratio_value in ratio_labels:
            key = f"{ratio_value:.4f}"
            subset = group[np.isclose(group["ratio_carrier"].astype(float), ratio_value, atol=1e-4)]
            if subset.empty:
                continue
            algo_map[key] = {
                "index_12": float(subset["index_12"].median()),
        ...
        priors[str(algorithm)] = algo_map
    return priors


def nearest_ratio_key(ratio_carrier: float, known_keys: list[str]) -> str:
    if not known_keys:
        return "_global"
    ratio_value = float(ratio_carrier)
    values = [float(key) for key in known_keys if key != "_global"]
    if not values:
        return "_global"
    nearest = min(values, key=lambda value: abs(value - ratio_value))
    return f"{nearest:.4f}"
...
def build_synth(row: dict, priors: dict[str, dict[str, dict[str, float]]]) -> FMSynth3:
    algorithm = str(row.get("algorithm", "series3_parallel2"))
    ratio_key = nearest_ratio_key(to_float(row.get("ratio_carrier"), 1.0), list(priors.get("_global", {}).keys()))
    algo_priors = priors.get(algorithm, {})
    prior = algo_priors.get(ratio_key) or algo_priors.get("_global") or priors["_global"].get(ratio_key) or priors["_global"]
    return FMSynth3(
        ...
        env_mod=FIXED_ENV_MOD,
        env_car=FIXED_ENV_CAR,
        env_scale1=1.0,
        env_scale2=1.0,
        env_scale3=1.0,
        env_scale4=1.0,
