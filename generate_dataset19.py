"""Generate an algorithm-focused FM-synthesis benchmark corpus for staged learning.

Architecture:
- Balanced sampling over algorithm/ratio combinations
- Structural FM indices remain non-zero so the algorithm topology is still audible
- Nuisance controls are narrowed aggressively to reduce within-class ambiguity
- Envelope shapes are sampled from a small set of structured archetypes
- Phase and carrier level are fixed because they inject unnecessary variability for the first cascade stage
- 4-second clips with sharded `int16` audio caches for easy transfer to remote training machines
- Renders audio with `fm_synth3`

Data flow:
- Input: configuration constants and environment overrides
- Output: `dataset_big19/parameters.csv`, `sample_*.wav`, `audio_big19_shards/shard_*.npy`, `audio_big19_manifest.json`, and `meta.json`
"""
...
OUTPUT_DIR = Path("dataset_big19")
SHARD_DIR = OUTPUT_DIR / "audio_big19_shards"
MANIFEST_PATH = OUTPUT_DIR / "audio_big19_manifest.json"
...
MIN_FREQUENCY = float(os.getenv("MIN_FREQUENCY", "70.0"))
...
# A smaller ratio schedule than big18 keeps the topology visible while reducing entropy.
RATIOS_DISCRETOS = (
    1 / 2,
    2 / 3,
    1.0,
...
TARGET_INFO = {
    "dataset": "dataset_big19",
    "description": (
        "algorithm-focused benchmark with a reduced ratio schedule, non-zero structural FM indices, "
        "narrow nuisance ranges, structured ADSR archetypes, fixed phase, and 4-second clips"
    ),
...
def choose_profile(weights: tuple[tuple[str, float], ...]) -> str:
    labels = [label for label, _ in weights]
    probs = [weight for _, weight in weights]
    return str(random.choices(labels, weights=probs, k=1)[0])


def sample_env_from_profile(profile: str, *, carrier: bool) -> Envelope:
    if profile == "pluck":
        attack = (0.0015, 0.010 if not carrier else 0.013)
        decay = (0.030, 0.10 if not carrier else 0.14)
        sustain = (0.06, 0.26 if not carrier else 0.40)
        release = (0.020, 0.12 if not carrier else 0.18)
    elif profile == "held":
        attack = (0.004, 0.020 if not carrier else 0.030)
        decay = (0.080, 0.18 if not carrier else 0.24)
        sustain = (0.52, 0.90 if not carrier else 0.96)
        release = (0.050, 0.20 if not carrier else 0.30)
    else:
        raise ValueError(f"Unknown envelope profile: {profile}")

    return Envelope(
        r(uniform_log(*attack)),
        r(uniform_log(*decay)),
        r(random.uniform(*sustain)),
        r(uniform_log(*release)),
        FIXED_CURVE,
        FIXED_CURVE,
        FIXED_CURVE,
    )
...
def sample_env_pair() -> tuple[Envelope, Envelope]:
    profile = choose_profile(ENV_PROFILE_WEIGHTS)
    return sample_env_from_profile(profile, carrier=False), sample_env_from_profile(profile, carrier=True)


def sample_index_active() -> float:
    x = random.random()
    if x < 0.60:
        return random.uniform(0.25, 1.6)
    if x < 0.90:
        return random.uniform(1.6, 3.2)
    return random.uniform(3.2, 4.5)
...
def build_row(sample_id: int, algorithm: str, ratio_carrier: float) -> dict:
    ...
    while True:
        ...
        env_mod, env_car = sample_env_pair()
        return {
            "id": sample_id,
            "algorithm": algorithm,
            ...
            "detune_carrier": r(random.uniform(-0.35, 0.35)),
            "feedback": r(random.uniform(0.0, 0.06)),
            "lfo_rate": r(random.uniform(0.0, 0.8)),
            "lfo_depth_cents": r(random.uniform(0.0, 2.0)),
            "key_scaling": r(random.uniform(0.0, 0.08)),
...
def render_shard(task: tuple[int, list[dict], int]) -> dict:
    ...
    return {
        "index": shard_idx,
        "file": str(Path("audio_big19_shards") / shard_filename(shard_idx)),
...
def write_meta(manifest: dict, rows: list[dict]) -> None:
    ...
    ratio_counts = {ratio_key(ratio): 0 for ratio in RATIOS_DISCRETOS}
    for row in rows:
        key = ratio_key(row["ratio_carrier"])
