"""Algorithm-focused FM classifier for `dataset_big10`, version 0_16.
...
- Lightweight waveform augmentation during training to reduce phase and alignment overfitting
...
OUTPUT_DIR = Path(os.getenv("OUTPUT_DIR", "model_training_big10_fmsynth3_0_16"))
MODEL_NAME = "model_training_big10_fmsynth3_0_16"
...
AUDIO_AUGMENTATION = os.getenv("AUDIO_AUGMENTATION", "1") == "1"
AUGMENT_SHIFT = int(os.getenv("AUGMENT_SHIFT", "256"))
AUGMENT_GAIN_MIN = float(os.getenv("AUGMENT_GAIN_MIN", "0.95"))
AUGMENT_GAIN_MAX = float(os.getenv("AUGMENT_GAIN_MAX", "1.05"))
AUGMENT_NOISE_STD = float(os.getenv("AUGMENT_NOISE_STD", "0.003"))
...
class MultiTaskSequence(tf.keras.utils.Sequence):
    ...
    def _augment_batch(self, batch: np.ndarray) -> np.ndarray:
        if not AUDIO_AUGMENTATION:
            return batch
        audio = np.asarray(batch[:, :, 0], dtype=np.float32).copy()
        if AUGMENT_SHIFT > 0:
            shifts = np.random.randint(-AUGMENT_SHIFT, AUGMENT_SHIFT + 1, size=audio.shape[0])
            for row_idx, shift in enumerate(shifts):
                if shift:
                    audio[row_idx] = np.roll(audio[row_idx], int(shift))
        if AUGMENT_GAIN_MIN != 1.0 or AUGMENT_GAIN_MAX != 1.0:
            gains = np.random.uniform(AUGMENT_GAIN_MIN, AUGMENT_GAIN_MAX, size=(audio.shape[0], 1)).astype(np.float32)
            audio *= gains
        if AUGMENT_NOISE_STD > 0:
            audio += np.random.normal(0.0, AUGMENT_NOISE_STD, size=audio.shape).astype(np.float32)
...
def build_model(
    ...
    algo_joint = Concatenate(name="algo_joint_concat")([algo_branch, ratio_branch, freq_branch, x])
    ...
    return Model(
        inputs=inputs,
        outputs=[algorithm_head, ratio_log2_head, freq_log2_head],
        name="compact_multirespectral_multitask_big10_0_13",
    )
...
def main() -> None:
    ...
    train_seq = MultiTaskSequence(audio_store, fit_audio_indices, y_fit, BATCH_SIZE, shuffle=True, augment=True)
    ...
    algo_pred = preds[0]
    ratio_log2_pred_scaled = preds[1]
