...
OUTPUT_DIR = "rnn_training_big4_fmsynth3_0_1"
MODEL_NAME = "rnn_training_big4_fmsynth3_0_1"
...
def build_model(
    ...
    if input_len % RNN_FRAME_SIZE != 0:
        raise ValueError(
            f"input_len={input_len} não é divisível por RNN_FRAME_SIZE={RNN_FRAME_SIZE}"
        )

    num_frames = input_len // RNN_FRAME_SIZE
    frame_dim = RNN_FRAME_SIZE * input_dims

    # Framing para reduzir passos temporais da RNN sem usar convolução/pooling.
    x_n = Reshape((num_frames, frame_dim), name="frame_reshape")(input_layer)
    x_n = BatchNormalization(name="input_batch_norm")(x_n)
    ...
    x_n = Dense(
        ...
        name="frame_embed_dense",
    )(x_n)
    x_n = Dropout(dropout_rnn, name="frame_embed_dropout")(x_n)
    ...
    return Model(input_layer, outputs, name="complete_multihead_rnn")
