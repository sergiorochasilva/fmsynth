...
Architecture:
- Balanced sampling over the full algorithm/ratio schedule used in the learnable `big12` formulation
- Structural FM indices remain non-zero so the algorithm topology stays audible
- Nuisance controls remain fixed, but envelope shapes keep controlled variation through structured archetypes
- Phase and carrier level are fixed because they inject unnecessary ambiguity for the first cascade stage
...
- Output: `dataset_big24/parameters.csv`, `sample_*.wav`, `audio_big24_shards/shard_*.npy`, `audio_big24_manifest.json`, and `meta.json`
"""
...
OUTPUT_DIR = Path("dataset_big24")
SHARD_DIR = OUTPUT_DIR / "audio_big24_shards"
MANIFEST_PATH = OUTPUT_DIR / "audio_big24_manifest.json"
...
# Keep the full ratio schedule from the learnable big12 corpus so exact topology remains observable.
RATIOS_DISCRETOS = (
    1 / 8,
    1 / 6,
    1 / 5,
...
TARGET_INFO = {
    "dataset": "dataset_big24",
    "description": (
        "algorithm-focused benchmark with full ratio coverage, non-zero structural FM indices, "
        "fixed nuisance controls, controlled envelope archetypes, fixed phase, and 4-second clips"
    ),
...
def sample_env_from_profile(profile: str, *, carrier: bool) -> Envelope:
    if profile == "pluck":
        attack = (0.0015, 0.010 if not carrier else 0.014)
        decay = (0.025, 0.10 if not carrier else 0.16)
        sustain = (0.04, 0.22 if not carrier else 0.34)
        release = (0.020, 0.12 if not carrier else 0.20)
    elif profile == "held":
        attack = (0.004, 0.020 if not carrier else 0.032)
        decay = (0.070, 0.18 if not carrier else 0.26)
        sustain = (0.48, 0.90 if not carrier else 0.97)
        release = (0.050, 0.20 if not carrier else 0.32)
    elif profile == "pad":
        attack = (0.015, 0.080 if not carrier else 0.110)
        decay = (0.100, 0.260 if not carrier else 0.360)
        sustain = (0.70, 0.98 if not carrier else 0.995)
        release = (0.120, 0.420 if not carrier else 0.550)
    else:
...
def render_shard(task: tuple[int, list[dict], int]) -> dict:
    ...
    return {
        "index": shard_idx,
        "file": str(Path("audio_big24_shards") / shard_filename(shard_idx)),
