"""Algorithm-focused FM classifier for `dataset_big10`, version 0_14.
...
- Auxiliary classification head for `ratio_carrier` and auxiliary regression for `frequencia_base`
...
OUTPUT_DIR = Path(os.getenv("OUTPUT_DIR", "model_training_big10_fmsynth3_0_14"))
MODEL_NAME = "model_training_big10_fmsynth3_0_14"
...
def build_model(
    ...
    return Model(
        inputs=inputs,
        outputs=[algorithm_head, ratio_class_head, freq_log2_head],
        name="compact_multirespectral_multitask_big10_0_14",
    )
...
def main() -> None:
    ...
    ratio_classes = build_ratio_classes(y_train_full["ratio_carrier"].to_numpy(dtype=np.float32))
    ratio_map = {float(value): idx for idx, value in enumerate(ratio_classes.tolist())}
    y_train_full["ratio_idx"] = assign_ratio_classes(y_train_full["ratio_carrier"].to_numpy(dtype=np.float32), ratio_classes)
    y_test_full["ratio_idx"] = assign_ratio_classes(y_test_full["ratio_carrier"].to_numpy(dtype=np.float32), ratio_classes)
    ...
    ratio_true = y_test_full["ratio_carrier"].to_numpy(dtype=np.float32)
    ratio_pred = ratio_classes[ratio_pred_idx]
    ...
    ratio_snap_accuracy = float(np.mean(ratio_pred_idx == y_test_full["ratio_idx"].to_numpy(dtype=np.int32)))
    ratio_mae = float(mean_absolute_error(ratio_true, ratio_pred))
    ratio_log2_mae = float(np.mean(np.abs(np.log2(ratio_pred) - np.log2(ratio_true))))
    ...
    results = {
        ...
        "final_metrics": {
            ...
            "ratio_snap_accuracy": ratio_snap_accuracy,
            "ratio_mae": ratio_mae,
