"""Conditioned split FM predictors for `dataset_big18`, version 0_1.
...
- Separate training modes with explicit conditioning and teacher forcing:
  ...
  - `env_mod`: ADSR regressors for the modulator envelope, conditioned on true `algorithm`, true `pitch`, and true `timbre`
  - `env_car`: ADSR regressors for the carrier envelope, conditioned on true `algorithm`, true `pitch`, and true `timbre`

Data flow:
- Input: `dataset_big18/parameters.csv` plus the contiguous prefix of rendered `sample_*.wav`
...
BASE_PATH = Path(os.getenv("DATASET_PATH", "dataset_big18"))
MODEL_PREFIX = "model_training_big18_fmsynth3_0_1"
...
GROUP_SPECS = {
    ...
    "env_mod": [
        {"head": "env_mod_attack_head", "column": "env_mod_attack", "transform": "log2", "loss_weight": 0.80, "group": "env_mod"},
        {"head": "env_mod_decay_head", "column": "env_mod_decay", "transform": "log2", "loss_weight": 0.80, "group": "env_mod"},
        {"head": "env_mod_sustain_head", "column": "env_mod_sustain", "transform": "identity", "loss_weight": 0.70, "group": "env_mod"},
        {"head": "env_mod_release_head", "column": "env_mod_release", "transform": "log2", "loss_weight": 0.80, "group": "env_mod"},
    ],
    "env_car": [
        {"head": "env_car_attack_head", "column": "env_car_attack", "transform": "log2", "loss_weight": 0.80, "group": "env_car"},
        {"head": "env_car_decay_head", "column": "env_car_decay", "transform": "log2", "loss_weight": 0.80, "group": "env_car"},
        {"head": "env_car_sustain_head", "column": "env_car_sustain", "transform": "identity", "loss_weight": 0.70, "group": "env_car"},
        {"head": "env_car_release_head", "column": "env_car_release", "transform": "log2", "loss_weight": 0.80, "group": "env_car"},
    ],
}
...
CONDITION_HEADS = {
    ...
    "env_mod": ["algorithm_onehot", "pitch_condition_input", "timbre_condition_input"],
    "env_car": ["algorithm_onehot", "pitch_condition_input", "timbre_condition_input"],
}
...
def load_audio_manifest(base_path: Path) -> dict | None:
    manifest_path = base_path / "audio_big18_manifest.json"
...
def build_model(input_len: int, n_algorithm_classes: int, target_group: str | None = None, target_specs: list[dict] | None = None) -> Model:
    ...
    if target_group == "algorithm":
    ...
    elif target_group in {"env_mod", "env_car"}:
        algorithm_condition_input = Input(shape=(n_algorithm_classes,), name="algorithm_condition_input")
        pitch_condition_input = Input(shape=(2,), name="pitch_condition_input")
        ...
        env_branch = Dense(max(DENSE_UNITS // 2, 112), activation="swish", name=f"{target_group}_branch_dense")(env_branch)
        env_branch = LayerNormalization(name="env_branch_ln")(env_branch)
        env_branch = Dropout(max(DROPOUT * 0.85, 0.05), name="env_branch_drop")(env_branch)
        for spec in target_specs:
            outputs.append(scalar_head(env_branch, spec["head"]))

    return Model(inputs=input_tensors, outputs=outputs, name=f"conditioned_split_fm_big18_{target_group}")
...
def group_stratify_key(frame: pd.DataFrame, target_group: str | None = None) -> pd.Series:
    ...
    elif target_group in {"timbre", "env_mod", "env_car"}:
        candidates.append(base + "__" + ratio_bucket.astype(str))
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train split FM predictors for dataset_big18.")
...
def main() -> None:
    ...
    if "algorithm" not in target_raw.columns:
        raise ValueError("dataset_big18 must contain an `algorithm` column.")
    ...
    if missing_columns:
        raise ValueError(f"dataset_big18 is missing required columns: {missing_columns}")
    ...
    if group == "algorithm":
        ...
        if group == "timbre":
            ...
            target_specs = GROUP_SPECS[group]
            condition_specs = GROUP_SPECS["pitch"] + GROUP_SPECS["timbre"]
        else:
            raise ValueError(f"Unsupported target group: {group}")
