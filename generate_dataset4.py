...
# Algoritmos disponíveis (mantido em sync com fm_synth3.py)
ALGORITHMS = [
    "series3_parallel2",
    "series3",
    "parallel5",
...
def sample_curve(style: str) -> str:
    if style == "musical":
        # Mais peso em exp, algumas variações
        return random.choices(CURVES, weights=[0.1, 0.6, 0.15, 0.15], k=1)[0]
...
def sample_env(style: str) -> Envelope:
    ...
    return Envelope(
        ...
        s,
        rls,
        curve_attack=sample_curve(style),
        curve_decay=sample_curve(style),
        curve_release=sample_curve(style),
    )
    ...
    with open(tmp_path, "w", encoding="utf-8", newline="") as f:
        ...
        for i in range(tamanho_dataset):
            ...
            while True:
                ...
                env1 = sample_env(style)
                env2 = sample_env(style)
                env3 = sample_env(style)
                env4 = sample_env(style)
                env5 = sample_env(style)
                env_carrier = sample_env(style)
            ...
            data = {
                ...
                "env1_curve_attack": env1.curve_attack,
                "env1_curve_decay": env1.curve_decay,
                "env1_curve_release": env1.curve_release,
                "env2_attack": r(env2.attack),
                "env2_decay": r(env2.decay),
                "env2_sustain": r(env2.sustain),
                "env2_release": r(env2.release),
                "env2_curve_attack": env2.curve_attack,
                "env2_curve_decay": env2.curve_decay,
                "env2_curve_release": env2.curve_release,
                "env3_attack": r(env3.attack),
                "env3_decay": r(env3.decay),
                "env3_sustain": r(env3.sustain),
                "env3_release": r(env3.release),
                "env3_curve_attack": env3.curve_attack,
                "env3_curve_decay": env3.curve_decay,
                "env3_curve_release": env3.curve_release,
                "env4_attack": r(env4.attack),
                "env4_decay": r(env4.decay),
                "env4_sustain": r(env4.sustain),
                "env4_release": r(env4.release),
                "env4_curve_attack": env4.curve_attack,
                "env4_curve_decay": env4.curve_decay,
                "env4_curve_release": env4.curve_release,
                "env5_attack": r(env5.attack),
                "env5_decay": r(env5.decay),
                "env5_sustain": r(env5.sustain),
                "env5_release": r(env5.release),
                "env5_curve_attack": env5.curve_attack,
                "env5_curve_decay": env5.curve_decay,
                "env5_curve_release": env5.curve_release,
                ...
                "env_carrier_curve_attack": env_carrier.curve_attack,
                "env_carrier_curve_decay": env_carrier.curve_decay,
                "env_carrier_curve_release": env_carrier.curve_release,
