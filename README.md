...
| [`generate_dataset.py`](/home/sergio/@pessoal/fmsynth/generate_dataset.py), [`generate_dataset2.py`](/home/sergio/@pessoal/fmsynth/generate_dataset2.py), [`generate_dataset3.py`](/home/sergio/@pessoal/fmsynth/generate_dataset3.py), [`generate_dataset4.py`](/home/sergio/@pessoal/fmsynth/generate_dataset4.py), [`generate_dataset5.py`](/home/sergio/@pessoal/fmsynth/generate_dataset5.py) | Dataset generators used by the experiments. |

## Suggested Reproducibility Flow

1. Generate the desired dataset with the corresponding script.
2. Train the autoencoder or the joint encoder/classifier model.
3. Generate embeddings with the encoding script if the experiment uses latents.
4. Train classification or regression models over the latent vectors.
5. Run `pred_nsynth*.py` to predict parameters from test audio.
6. Run `resynth_nsynth*.py` to synthesize audio from the predicted parameters.
7. Use `evaluating.py` to measure the distance between original and resynthesized audio.
