...
- Loads a two-stage hierarchical algorithm predictor:
  ...
  - `exact`: fine 4-way algorithm classifier conditioned on the predicted family
...
from model_training_big16_fmsynth3_0_1 import GROUP_SPECS as HIER_GROUP_SPECS
from model_training_big16_fmsynth3_0_1 import MODEL_PREFIX as HIER_MODEL_PREFIX
from model_training_big16_fmsynth3_0_1 import build_model as build_hier_model
...
MODEL_NAME = "model_training_big16_fmsynth3_0_1"
...
DEFAULT_OUTPUT_DIR = Path("nsynth-pred-big16_0_1")
...
def load_hier_model(stage: str, audio_len: int):
    ...
    n_classes = 2 if stage == "family" else 4
...
def main() -> None:
    ...
    for start in range(0, len(wav_files), args.batch_size):
        ...
        exact_pred = np.asarray(
            exact_model.predict({"audio_input": x_batch, "family_condition_input": family_onehot}, batch_size=len(batch_files), verbose=0),
            dtype=np.float32,
        )
        exact_idx = np.argmax(exact_pred, axis=1)
        ...
        for idx, wav_path in enumerate(batch_files):
            ...
            row = {
                ...
                "algorithm_prob": float(np.max(exact_pred[idx])),
            }
