"""Generate a compact FM-synthesis benchmark corpus for fast iteration.

Architecture:
- Balanced sampling over a reduced target space with fixed phase, fixed envelope curves, and a small algorithm set
...
- Output: `dataset_big8/parameters.csv`, `sample_*.wav`, `audio_big8_shards/shard_*.npy`, `audio_big8_manifest.json`, and `meta.json`
"""
...
OUTPUT_DIR = Path("dataset_big8")
SHARD_DIR = OUTPUT_DIR / "audio_big8_shards"
MANIFEST_PATH = OUTPUT_DIR / "audio_big8_manifest.json"
CSV_PATH = OUTPUT_DIR / "parameters.csv"
META_PATH = OUTPUT_DIR / "meta.json"

DATASET_SIZE = int(os.getenv("DATASET_SIZE", "3072"))
...
TARGET_INFO = {
    "dataset": "dataset_big8",
    "description": "compact benchmark with reduced target ambiguity",
...
def sample_algorithm_schedule(n: int) -> list[str]:
    base = n // len(ALGORITHMS)
    rem = n % len(ALGORITHMS)
    schedule: list[str] = []
    for idx, algo in enumerate(ALGORITHMS):
        schedule.extend([algo] * (base + (1 if idx < rem else 0)))
    random.shuffle(schedule)
    return schedule


def sample_ratio_carrier() -> float:
    return random.choices(
        population=RATIOS_DISCRETOS,
        weights=[6, 4, 5, 5, 4, 4, 4, 8, 6, 6, 5, 4],
        k=1,
    )[0]


def sample_index_12() -> float:
    x = random.random()
    if x < 0.55:
        return random.uniform(0.0, 2.0)
    if x < 0.9:
        return random.uniform(2.0, 4.5)
    return random.uniform(4.5, 6.0)


def sample_detune_carrier() -> float:
    return random.uniform(-12.0, 12.0)


def sample_feedback() -> float:
    if random.random() < 0.65:
        return random.uniform(0.0, 0.12)
    return random.uniform(0.12, 0.28)


def sample_lfo() -> tuple[float, float]:
    if random.random() < 0.6:
        return 0.0, 0.0
    return random.uniform(0.5, 4.0), random.uniform(0.0, 8.0)


def sample_key_scaling() -> float:
...
def build_row(sample_id: int, algorithm: str) -> dict:
    attempts = 0
    while True:
        attempts += 1
        if attempts > 200:
        ...
        ratio_carrier = sample_ratio_carrier()
        if not valid_configuration(fc, ratio_carrier):
            continue

        index_12 = sample_index_12()
        detune_carrier = sample_detune_carrier()
        feedback = sample_feedback()
        lfo_rate, lfo_depth_cents = sample_lfo()
        key_scaling = sample_key_scaling()
        return {
            "id": sample_id,
            "algorithm": algorithm,
...
def build_synth_from_row(row: dict) -> tuple[FMSynth3, float, str]:
    ...
    synth = FMSynth3(
        ...
        index_23=FIXED_INDEX_23,
        index_3c=FIXED_INDEX_3C,
        index_4c=FIXED_INDEX_4C,
        index_5c=FIXED_INDEX_5C,
        env_mod=env_mod,
        env_car=env_car,
        env_scale1=1.0,
        env_scale2=1.0,
...
def write_csv(csv_path: Path) -> list[dict]:
    schedule = sample_algorithm_schedule(DATASET_SIZE)
    rows: list[dict] = []
    ...
    with open(tmp_path, "w", encoding="utf-8", newline="") as f:
        ...
        for sample_id in range(DATASET_SIZE):
            ...
            row = build_row(sample_id, schedule[sample_id])
            writer.writerow(row)
            rows.append(row)
...
def write_audio(rows: list[dict], audio_sample_len: int) -> dict:
    ...
    for shard_idx in range(n_shards):
        ...
        shards_meta.append(
            {
                "index": shard_idx,
                "file": str(Path("audio_big8_shards") / shard_filename(shard_idx)),
