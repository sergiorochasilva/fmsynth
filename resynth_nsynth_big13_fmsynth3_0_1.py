"""Resynthesize NSynth audio from `big13` predictions.

Architecture:
- Loads predicted algorithm, ratio, carrier frequency, structural controls, and envelopes from the `big13` model
- Fills any missing controls using corpus priors
- Renders waveforms with `FMSynth3`

Data flow:
- Input: predicted parameter JSON and NSynth metadata
- Output: resynthesized `.wav` files in `nsynth-pred-big13/`
"""
...
DEFAULT_PRED_JSON = MODEL_DIR / "nsynth-pred-big13" / f"params_pred_nsynth_{MODEL_NAME}.json"
DEFAULT_NSYNTH_AUDIO_DIR = Path("nsynth-test/audio")
DEFAULT_EXAMPLES_JSON = Path("nsynth-test/examples.json")
DEFAULT_OUTPUT_DIR = Path("nsynth-pred-big13")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Resynthesize NSynth using big13 predictions.")
