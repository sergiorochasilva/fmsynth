"""Deep multi-head hybrid regressor for `dataset_big6` (variant 0_3).

Architecture:
- Raw waveform input loaded from sharded `int16` audio or legacy caches
- Dual-branch encoder with a deeper 1D residual waveform tower and a 2D log-spectrogram tower
- Shared dense trunk plus specialized heads for grouped numeric targets and categorical targets

Data flow:
- Input: `dataset_big6/parameters.csv` plus `audio_big6_manifest.json` shards or `sample_*.wav`
...
MODEL_NAME = "model_training_big6_fmsynth3_0_3"
...
# Batch maior para ocupar mais VRAM e aproximar 8-12 GB em GPUs de 20 GB.
BATCH_SIZE = int(os.getenv("TRAIN_BATCH_SIZE", "24"))
...
STFT_FRAME_LENGTH = int(os.getenv("STFT_FRAME_LENGTH", "1024"))
STFT_FRAME_STEP = int(os.getenv("STFT_FRAME_STEP", "256"))
STFT_FFT_LENGTH = int(os.getenv("STFT_FFT_LENGTH", "1024"))
MEL_BINS = int(os.getenv("MEL_BINS", "128"))
...
def log_mel_spectrogram_3d(audio_batch):
    """Convert `(batch, time, 1)` audio to `(batch, frames, mel_bins, 1)` log-mel features."""
    mono = tf.squeeze(audio_batch, axis=-1)
    stft = tf.signal.stft(
        mono,
    ...
    mel = tf.tensordot(power, MEL_WEIGHT_MATRIX, axes=1)
...
AUDIO_SAMPLE_RATE = int(meta.get("sample_rate_out", 16000))
tamanho_dataset = int(meta.get("tamanho_dataset", 5000))
MEL_WEIGHT_MATRIX = tf.constant(
    tf.signal.linear_to_mel_weight_matrix(
        num_mel_bins=MEL_BINS,
        num_spectrogram_bins=STFT_FFT_LENGTH // 2 + 1,
        sample_rate=AUDIO_SAMPLE_RATE,
        lower_edge_hertz=20.0,
        upper_edge_hertz=float(AUDIO_SAMPLE_RATE) / 2.0,
    ),
    dtype=tf.float32,
)
...
print(
    "Spectral config: "
    f"audio_sample_rate={AUDIO_SAMPLE_RATE}, stft_frame_length={STFT_FRAME_LENGTH}, "
    f"stft_frame_step={STFT_FRAME_STEP}, mel_bins={MEL_BINS}"
)
...
def residual_conv2d_block(
    ...
    *,
    ...
    y = Activation(activation, name=f"{block_name}_act1")(y)
    if dropout_rate > 0:
        y = SpatialDropout2D(dropout_rate, name=f"{block_name}_drop1")(y)
    y = Conv2D(
        filters,
        kernel_size=kernel_size,
        padding="same",
...
def build_model(
    ...
    def pooled_2d(tensor, prefix):
        gap = GlobalAveragePooling2D(name=f"{prefix}_gap")(tensor)
        gmp = GlobalMaxPooling2D(name=f"{prefix}_gmp")(tensor)
        return Concatenate(name=f"{prefix}_pool_concat")([gap, gmp])
    ...
    wave = SpatialDropout1D(0.06, name="wave_stem_dropout")(wave)
    ...
    # Spectrogram branch
    spec = Lambda(log_mel_spectrogram_3d, name="log_mel_spectrogram")(input_layer)
    spec = BatchNormalization(name="spec_batch_norm")(spec)
    spec = Conv2D(
        filters=32,
        kernel_size=(5, 5),
        padding="same",
    ...
    spec = SpatialDropout2D(0.08, name="spec_dropout_1")(spec)
    spec = MaxPooling2D(pool_size=(2, 2), name="spec_pool_1")(spec)
    spec_scale1 = spec

    spec = residual_conv2d_block(
        spec,
        filters=64,
        kernel_size=(3, 3),
        activation=activation,
        bias_cnn=bias_cnn,
        kernel_regularizer_cnn=kernel_regularizer_cnn,
        block_name="spec_res_1",
        dropout_rate=0.08,
    )
    spec = MaxPooling2D(pool_size=(2, 2), name="spec_pool_2")(spec)
    spec_scale2 = spec
    ...
    spec = residual_conv2d_block(
        ...
        bias_cnn=bias_cnn,
        kernel_regularizer_cnn=kernel_regularizer_cnn,
        block_name="spec_res_2",
        dropout_rate=0.1,
    )
    spec = MaxPooling2D(pool_size=(2, 2), name="spec_pool_3")(spec)
    ...
    features = Concatenate(name="feature_fusion")(
        [
            ...
            pooled_2d(spec_scale1, "spec_s1"),
            pooled_2d(spec_scale2, "spec_s2"),
            pooled_2d(spec, "spec_final"),
        ]
    )
    ...
    shared_head = Dropout(max(dropout_head * 0.5, 0.1), name="shared_head_dropout_2")(shared_head)
    shared_head = Dense(
        192,
        activation=activation,
        use_bias=bias_head,
    ...
    shared_head = Dropout(max(dropout_head * 0.35, 0.08), name="shared_head_dropout_3")(shared_head)
    shared_head = Dense(
        160,
        activation=activation,
        use_bias=bias_head,
        name="shared_head_dense_4",
    )(shared_head)
    ...
    for head_name, output_dim in numeric_output_dims.items():
        ...
        reg_hidden = Dropout(max(dropout_head * 0.4, 0.08), name=f"{head_name}_dropout")(reg_hidden)
        reg_hidden = Dense(
            128,
            activation=activation,
            use_bias=bias_head,
    ...
    if has_freq_head:
        ...
        freq_hidden = Dropout(max(dropout_head * 0.35, 0.05), name="freq_head_dropout")(freq_hidden)
    ...
    for col, n_classes in categorical_output_dims.items():
        ...
        cat_hidden = Dropout(max(dropout_head * 0.3, 0.05), name=f"{head_name}_dropout")(cat_hidden)
    ...
    return Model(input_layer, outputs, name="complete_multihead_hybrid_0_3")
