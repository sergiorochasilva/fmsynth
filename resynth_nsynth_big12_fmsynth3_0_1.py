...
- Fills the remaining FM structural indices using corpus priors
...
def build_prior_table(dataset_params: Path) -> dict[str, dict[str, float]]:
    ...
    priors["_global"] = {
        "index_12": float(df["index_12"].median()),
        "index_23": float(df["index_23"].median()),
        "index_3c": float(df["index_3c"].median()),
        "index_4c": float(df["index_4c"].median()),
        "index_5c": float(df["index_5c"].median()),
    }
    return priors
