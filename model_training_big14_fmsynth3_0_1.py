"""Split FM predictors for `dataset_big13`, version 0_1.
...
- Separate training modes for:
  ...
  - `carrier`: carrier ratio / base frequency / detune regressors
  - `modulators`: FM indices, feedback, LFO, key scaling, and ADSR regressors
...
MODEL_PREFIX = "model_training_big14_fmsynth3_0_1"
...
def build_model(input_len: int, n_algorithm_classes: int, target_group: str | None = None, target_specs: list[dict] | None = None) -> Model:
    ...
    if target_group == "algorithm":
        ...
        carrier_branch = Dense(max(DENSE_UNITS // 2, 96), activation="swish", name="carrier_branch_dense")(x)
        ...
        mod_branch = Dense(max(DENSE_UNITS // 2, 96), activation="swish", name="mod_branch_dense")(x)
    ...
    return Model(inputs=inputs, outputs=outputs, name=f"split_fm_big14_{target_group}")
...
def main() -> None:
    ...
    if group == "algorithm":
        y_fit["algorithm_head"] = y_train_model["algorithm_idx"].to_numpy(dtype=np.int32)[fit_idx]
        y_val["algorithm_head"] = y_train_model["algorithm_idx"].to_numpy(dtype=np.int32)[val_idx]
        y_test["algorithm_head"] = y_test_full["algorithm_idx"].to_numpy(dtype=np.int32)
    else:
        for spec in specs:
    ...
    class SplitSequence(tf.keras.utils.Sequence):
        def __init__(self, audio: np.ndarray, sample_indices: np.ndarray, y: dict[str, np.ndarray], batch_size: int, shuffle: bool = True):
        ...
        def __getitem__(self, idx):
            ...
            x_batch = normalize_audio_batch(self.audio[audio_ids]).reshape(len(audio_ids), audio_len, 1)
