"""Algorithm-focused FM classifier for `dataset_big12`, version 0_2.

Architecture:
- Raw waveform input
- Differentiable multi-resolution log-mel front-end inside the model, with extra temporal derivatives
- Slightly wider and deeper residual 1D CNN backbone over the time axis of the stacked spectrogram features
- Shared dense trunk with a direct `algorithm` classifier
- Auxiliary regression heads for `ratio_carrier` and `frequencia_base`

Data flow:
- Input: `dataset_big12/parameters.csv` plus `audio_big12_manifest.json` shards or `sample_*.wav`
...
BASE_PATH = Path(os.getenv("DATASET_PATH", "dataset_big12"))
OUTPUT_DIR = Path(os.getenv("OUTPUT_DIR", "model_training_big12_fmsynth3_0_2"))
MODEL_NAME = "model_training_big12_fmsynth3_0_2"
...
DROPOUT = float(os.getenv("DROPOUT", "0.04"))
BASE_FILTERS = int(os.getenv("BASE_FILTERS", "80"))
CNN_BLOCKS = int(os.getenv("CNN_BLOCKS", "7"))
DENSE_UNITS = int(os.getenv("DENSE_UNITS", "448"))
...
AUDIO_MANIFEST_PATH = BASE_PATH / "audio_big12_manifest.json"
AUDIO_LEGACY_CACHE_PATH = BASE_PATH / "audio_big12_int16.npy"
...
def build_model(
    ...
    return Model(
        ...
        name="compact_multirespectral_multitask_big12_0_1",
    )
...
def main() -> None:
    ...
    if not required_columns.issubset(target_raw.columns):
        raise ValueError("dataset_big12 must contain an `algorithm` column.")
    ...
    y_train_full.to_csv(OUTPUT_DIR / "y_train_big12.csv", index=False)
    y_test_full.to_csv(OUTPUT_DIR / "y_test_big12.csv", index=False)
